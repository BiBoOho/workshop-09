...
// ຕົວແປສຳລັບຄວບຄຸມການເຮັດວຽກ

let countDownTitle = '';
let countDownDate = '';

let countDownValue = Date; //ເກັບວັນທີ່ເລຶອກຈາກຟອມ
let countDownActive; //ຕົວຈັບເວລາ
let saveCountDown; // ເກັບຂໍ້ມູນຫົວຂໍ້ ແລະ ວັນແຈ້ງເຕືອນ (object)
...
function updateCountDown(e) {
    ...
    if(countDownTitle === '') {
        ...
        countDownValue = new Date(countDownDate).getTime();//ເວລາທີ່ຕັ້ງໄວ້
        setUpTime();
    }
}
...
function setUpTime() {
    countDownActive = setInterval(()=>{
        ...
        if(distance<0){
            ...
            //ສະແດງຜົນ Complete
            completeInfoEl.textContent = `${countDownTitle} ວັນທີ ${countDownDate}`; 
